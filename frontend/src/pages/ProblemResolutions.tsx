...
export default function ProblemResolutions() {
  ...
  const accepted = mockSubmissions.filter(s => s.status === 'accepted');
  return (
    <PageContainer>
      ...
      <div className="grid gap-4">
        {accepted.map(s => (
          <Card key={s.id} className="border-border/50 bg-card/50 cursor-pointer hover:border-primary/50" onClick={() => setSelectedCode(s.code || '// Código não disponível')}>
      ...
      <Dialog open={!!selectedCode} onOpenChange={() => setSelectedCode(null)}>
        <DialogContent className="max-w-3xl max-h-[80vh]"><DialogHeader><DialogTitle>Código da Solução</DialogTitle></DialogHeader><div className="h-[400px]"><CodeEditor value={selectedCode || ''} onChange={() => {}} language="python" onLanguageChange={() => {}} readOnly /></div></DialogContent>
      </Dialog>
    </PageContainer>
  );
}
